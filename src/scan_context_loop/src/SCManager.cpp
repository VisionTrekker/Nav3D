...
void SCManager::detectLoopClosureID( int _query_index, int& _loop_id, float& _yaw_diff )
{
    _loop_id = -1;
    _yaw_diff = 0.0f;

    auto curr_key = polarcontext_invkeys_mat_[_query_index];
    auto curr_desc = polarcontexts_[_query_index];
    ...
    {
        _loop_id = nn_idx;
        cout << "[Loop found] Nearest distance: " << min_dist << " btn " << _query_index << " and " << nn_idx << "." << endl;
    }
    else
    {
        cout << "[Not loop] Nearest distance: " << min_dist << " btn " << _query_index << " and " << nn_idx << "." << endl;
    }

    _yaw_diff = deg2rad(nn_align * PC_UNIT_SECTORANGLE);

} // SCManager::detectLoopClosureID (3-arg overload)
